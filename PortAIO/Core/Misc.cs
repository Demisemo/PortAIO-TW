...
namespace PortAIO.Dual_Port
{
    class Misc
    {
        ...
        {
            (info = new Menu("PortAIO - 訊息", "PAIOInfo", true)).AddToMainMenu();
            ...
            var dualPort = new Menu("雙重-移植", "DualPAIOPort");
            ...
            switch (ObjectManager.Player.Hero)
            {
                ...
                case EloBuddy.Champion.Ahri:
                    champ = new string[] { "OKTW", "DZAhri", "EloFactory Ahri", "KappaSeries", "xSalice", "BadaoSeries", "M1D 0R F33D", "AhriSharp", "[SDK] Flowers' Series", "Babehri", "EasyAhri", "SenseAhri" };
                    break;
                ...
                case EloBuddy.Champion.LeeSin:
                    champ = new string[] { "ElLeeSin", "BrianSharp", "FreshBooster", "Hikicarry LeeSin", "Lee is Back", "Slutty LeeSin", "[SDK] Valvrave#", "yol0LeeSin", "[SDK] Tc_SDKexAIO", "LeeSin EloClimber", "MasterOfInsec" };
                    break;
                ...
                case EloBuddy.Champion.Orianna:
                    champ = new string[] { "Kortatu Orianna", "OKTW", "SAutoCarry", "SFXChallenger", "xSalice", "[SDK] ExorAIO", "Orianna - the Ruler of Ball", "Orianna by Trelli", "Orianna Grande", "Midlane#" };
                    break;
                ...
                case EloBuddy.Champion.Riven:
                    champ = new string[] { "KurisuRiven", "Hoola Riven", "SAutoCarry", "Nechrito Riven", "[SDK] Flowers' Series", "[SDK] ReforgedRiven", "EasyPeasyRivenSqueezy", "EloFactory Riven", "Flowers' Riven", "HeavenStrikeRiven", "yol0Riven", "RivenToTheChallenger" };
                    break;
            ...
            if (hasDualPort)
            {
                dualPort.AddItem(new MenuItem(ObjectManager.Player.Hero.ToString(), "選擇您要的腳本?").SetValue(new StringList(champ)));
            }
            ...
            dutility.AddItem(new MenuItem("Predictioner", "選擇: 預判?").SetValue(new StringList(new[] { "SPredictioner", "OKTWPredictioner" })));

            dutility.AddItem(new MenuItem("enableCamera", "啟用 切換攝像頭?").SetValue(false));
            dutility.AddItem(new MenuItem("Camera", "選擇: 切換攝像頭?").SetValue(new StringList(new[] { "HSCamera" })));
            ...
            var autoPlay = new Menu("自動掛機", "PortAIOAUTOPLAY");
            autoPlay.AddItem(new MenuItem("AutoPlay", "啟用 自動掛機?").SetValue(false));
            autoPlay.AddItem(new MenuItem("selectAutoPlay", "選擇: 自動掛機?").SetValue(new StringList(new[] { "AramDETFull" /*,"AutoJungle", "SharpAI"*/ })));
