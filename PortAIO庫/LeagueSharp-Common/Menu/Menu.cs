namespace LeagueSharp.Common
{
    ...
    public class Menu
    {
        ...
        static Menu()
        {
            ...
            Root.AddItem(
                new MenuItem("LeagueSharp.Common.TooltipDuration", "提示通知時間").SetValue(
                    new Slider(1500, 0, 5000)));
            Root.AddItem(
             new MenuItem("Menu.Compact", "緊湊菜單").SetValue(false));

            Root.AddItem(
                new MenuItem("FontInfo", "需要按下F5進行修改 翻譯:Demsiemo").SetFontStyle(FontStyle.Bold, Color.Yellow));
