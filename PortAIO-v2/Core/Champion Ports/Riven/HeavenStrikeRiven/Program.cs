 ...
 namespace HeavenStrikeRiven
{
    public class Program
    {
        ...
        private static void OnStart()
        {
            ...
            Menu spellMenu = Menu.AddSubMenu(new Menu("技能", "Spells"));
            spellMenu.AddItem(new MenuItem("RcomboAlways", "使用 R").SetValue(false));
            spellMenu.AddItem(new MenuItem("RcomboKillable", "使用開R搶頭").SetValue(true));
            spellMenu.AddItem(new MenuItem("R2comboKS", "使用R2搶頭").SetValue(true));
            spellMenu.AddItem(new MenuItem("R2comboMaxdmg", "R的最大傷害").SetValue(true));
            spellMenu.AddItem(new MenuItem("R2 Badao Style", "R2 Badao 邏輯").SetValue(true));
            spellMenu.AddItem(new MenuItem("Ecombo", "使用 E").SetValue(true));
            spellMenu.AddItem(new MenuItem("Q Gap", "Q 防突進").SetValue(false));
            spellMenu.AddItem(new MenuItem("Use Q Before Expiry", "Q技能剩餘時間使用").SetValue(true));
            spellMenu.AddItem(new MenuItem("Q strange Cancel", "Q 無動畫取消").SetValue(true));
            spellMenu.AddItem(new MenuItem("Qmode", "Q 模式").SetValue(new StringList(new[] { "鎖定目標", "鼠標位置" }, 0)));
            Menu BurstCombo = spellMenu.AddSubMenu(new Menu("爆發連招", "Burst Combo"));
            //BurstCombo.AddItem(new MenuItem("Burst", "Burst").SetValue(new KeyBind('T', KeyBindType.Press)));
            BurstCombo.AddItem(new MenuItem("Use Flash", "使用閃現").SetValue(false));
            Menu Misc = Menu.AddSubMenu(new Menu("雜項", "Misc"));
            Misc.AddItem(new MenuItem("W interrupt", "W 中斷技能").SetValue(true));
            Misc.AddItem(new MenuItem("W gapcloser", "W 防突進").SetValue(true));
            Menu Draw = Menu.AddSubMenu(new Menu("顯示", "Draw"));
            Draw.AddItem(new MenuItem("Draw dmg text", "顯示傷害文字").SetValue(true));
            Menu other = Menu.AddSubMenu(new Menu("其他", "other"));
            other.AddItem(new MenuItem("Flee", "逃跑").SetValue(new KeyBind("Z".ToCharArray()[0], KeyBindType.Press)));
            other.AddItem(new MenuItem("WallJumpHelper", "過牆").SetValue(new KeyBind("A".ToCharArray()[0], KeyBindType.Press)));
            //other.AddItem(new MenuItem("FastHarass", "FastHarass").SetValue(new KeyBind("Y".ToCharArray()[0], KeyBindType.Press)));
            Menu Clear = Menu.AddSubMenu(new Menu("清線", "Clear"));
            Clear.AddItem(new MenuItem("Use Tiamat", "使用海神斧").SetValue(true));
