...
namespace SharedExperience
{
    class Program
    {
        ...
        public static void OnGameLoad(EventArgs args)
        {
            ...
            Chat.Print("<font color=\"#00BFFF\">Shared Experience</font> <font color=\"#FFFFFF\"> - Loaded</font>");
        }
        ...
        public static void OnGameUpdate(EventArgs args)
        {
            ...
            foreach (AIHeroClient hero in ObjectManager.Get<AIHeroClient>())
            {
                ...
                foreach (AIHeroClient enemy in ObjectManager.Get<AIHeroClient>())
                {
                    ...
                    {
                        if (enemy.IsHPBarRendered) VisibleCount[i] += 1;
                        if (enemy.IsDead)
                        {
                            VisibleCount[i] -= 1;
                            deadCount += 1;
        ...
        public static void Drawing_OnDraw(EventArgs args)
        {
            ...
            foreach (AIHeroClient hero in ObjectManager.Get<AIHeroClient>())
            {
                ...
                if ((Environment.TickCount - TimeChangedExp[i]) < 5000 && InvisibleCount[i] > 0)
                {
                    if (menu.Item("drawPredictionCircle").GetValue<bool>()) Drawing.DrawCircle(LastMinionPosition[i], 1500, Color.Red);
