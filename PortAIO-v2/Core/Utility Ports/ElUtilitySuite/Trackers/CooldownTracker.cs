    ...
    internal class CoolDownTimers : IPlugin
    {
        ...
        private void OnDrawingEndScene(EventArgs args)
        {
            try
            {
                ...
                var outline = this.Menu.Item("Outline").GetValue<bool>();
                var offsetTop = this.Menu.Item("OffsetTop").GetValue<Slider>().Value;
                var offsetLeft = this.Menu.Item("OffsetLeft").GetValue<Slider>().Value;

                foreach (var ability in
                    this._drawings.Where(d => d.Object.IsValid && d.Position.IsOnScreen() && d.End > Game.Time))
                {
        ...
        private void OnGameObjectCreate(GameObject sender, EventArgs args)
        {
            try
            {
                if (!sender.IsValid || sender.Type != GameObjectType.obj_GeneralParticleEmitter
                    || sender.IsMe && !this.Menu.Item("Self").GetValue<bool>()
                    || sender.IsAlly && !this.Menu.Item("Ally").GetValue<bool>()
                    || sender.IsEnemy && !this.Menu.Item("Enemy").GetValue<bool>())
                {
                    return;
                }
