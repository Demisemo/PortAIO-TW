 ...
 namespace Valvrave_Sharp.Plugin
{
    ...
    using Valvrave_Sharp.ValvEvade;
    ...
    internal class Zed : Program
    {
            ...
            if (MainMenu["KillSteal"]["Q"] && Q.IsReady())
            {
                var targets =
                    Variables.TargetSelector.GetTargets(Q.Range + Q.Width / 2 + RangeTarget, Q.DamageType)
                        .Where(i => !IsKillByMark(i) && i.Health + i.AttackShield <= Q.GetDamage(i))
                        .ToList();
                if (targets.Count > 0)
                {
        ...
        private static void OnDraw(EventArgs args)
        {
            ...
            if (MainMenu["Draw"]["UseR"] && R.Level > 0)
            {
                ...
                Drawing.DrawText(
                    pos.X - (float)Drawing.GetTextEntent((text), 15).Width / 2,
                    pos.Y + 20,
            ...
            if (MainMenu["Draw"]["DMark"] && RShadow.IsValid())
            {
                ...
                if (target != null)
                {
                    ...
                    Drawing.DrawText(pos.X - (float)Drawing.GetTextEntent((text), 15).Width / 2, pos.Y + 40, Color.Red, text);
                }
            }
