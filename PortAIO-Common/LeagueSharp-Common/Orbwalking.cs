...
//            }
        ...
        public class Orbwalker : IDisposable
        {
            ...
            public Orbwalker(Menu attachToMenu)
            {
                ...
                drawings.AddItem(
                    new MenuItem("AACircle", "自身普攻範圍").SetShared()
                        .SetValue(new Circle(true, Color.FromArgb(155, 255, 255, 0))));
                drawings.AddItem(
                    new MenuItem("AACircle2", "敵人普攻範圍").SetShared()
                ...
                misc.AddItem(
                    new MenuItem("HoldPosRadius", "保持半徑位置").SetShared().SetValue(new Slider(50, 50, 250)));
                misc.AddItem(new MenuItem("PriorizeFarm", "尾兵先於騷擾").SetShared().SetValue(true));
                ...
                _config.AddItem(new MenuItem("FarmDelay", "農兵延遲").SetShared().SetValue(new Slider(0, 0, 200)));

                /*Load the menu*/
                _config.AddItem(
                    new MenuItem("LastHit", "農兵").SetShared().SetValue(new KeyBind('X', KeyBindType.Press)));

                _config.AddItem(new MenuItem("Farm", "混合").SetShared().SetValue(new KeyBind('C', KeyBindType.Press)));

                _config.AddItem(
                    new MenuItem("Freeze", "凍結").SetShared().SetValue(new KeyBind('N', KeyBindType.Press)));
                ...
                _config.AddItem(
                    new MenuItem("StillCombo", "連招無需移動").SetShared()
